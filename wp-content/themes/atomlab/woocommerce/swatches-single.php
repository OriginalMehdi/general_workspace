...
<form <?php echo( $html ); ?>
	...
	if ( empty( $available_variations ) && false !== $available_variations ) {
		?>
		<p class="stock out-of-stock"><?php esc_html__( 'This product is currently out of stock and unavailable.', 'atomlab' ) ?></p>
	<?php } else {
		?>
		<table class="variations" cellspacing="0">
			...
			foreach ( $attributes as $attribute_name => $options ) {
				...
				if ( $options['is_variation'] == '1' ) {
					...
					if ( isset( $settings['isw_attr'] ) && is_array( $settings['isw_attr'] ) && in_array( Insight_Attribute_Swatches_Utils::utf8_urldecode( $attribute_name ), $settings['isw_attr'] ) ) {
			...
			foreach ( $isw_attributes as $attribute_idx => $attribute_name ) {
				...
				<tr>
					<td class="label">
						<label
							for="<?php echo sanitize_title( $title ); ?>"><?php echo( $curr['title'] ? $curr['title'] : wc_attribute_label( $title ) ); ?></label>
					</td>
					<td class="value">
						<div
								...
								case 'isw_color':
									foreach ( $curr['terms'] as $l => $b ) {
										...
										<div
											...
											<?php echo $b->name; ?>
											<span
												style="background-color: <?php echo $curr['custom'][ $b->slug ]; ?>"></span>
										</div>
										<?php
									}
									break;
						...
						if ( $cnt == count( $isw_attributes ) ) {
							?>
							<a class="reset_variations" href="#"><?php esc_html_e( 'Clear', 'atomlab' ); ?></a>
							<?php
						}
